...
|---|---|---|---|---|
| **C1** — ~84% resolved w/o human intervention at quality threshold | **SUPPORTED** (mature 83.92% reconstructs, net of wrong-autos) | n/a | **NOT SUPPORTED** (fresh 70.11%; -13.8 pts; wrong-auto ~3x) | n/a |
...
- Mature: auto-resolved incl. wrong = 84.5417%; wrong-auto among auto-resolved = 0.7393%. Net success = 84.5417% x (1 - 0.007393) = **83.92%** (matches `successful_automated_resolution_pct.full_meridian`).
- Fresh: 71.6667% x (1 - 0.021705) = **70.11%**.
...
|---|---|---|---|---|
| Management narrow metric (**C4**) | Inference for successful resolutions only | **$0.0915** | **$0.0930** | 1.02x |
| Model + retrieval + orchestration incl. retries & failed attempts | Full model/infra to *produce* the successes | **$0.1570** | **$0.1949** | 1.24x |
| Fully loaded incl. variable human review & fallback labor | Above + human-in-the-loop cost | **$0.4270** | **$0.8141** | **1.91x** |
...
- Model/infra is **~1.72x (mature) / ~2.10x (fresh)** the narrow metric.
- Fully loaded is **~4.67x (mature) / ~8.76x (fresh)** the narrow metric, and **nearly doubles** from mature to fresh (driven by the higher fallback/human-review incidence on lower-containment fresh customers).
...
| E2 | Fresh success 70.11%; wrong-auto ~3x | `fresh_customer_holdout.successful_automated_resolution_pct.full_meridian` (70.111); `...wrong_auto_errors...` (2.171) |
...
7. **Independent re-execution** of `Sample_Evaluation_Harness.py` on raw case-level data to confirm the reconstructions in sections 3–6 end-to-end.
